...
$(document).ready(function () {
  ...
  var ptLayer =  new L.FeatureGroup();
  map.addLayer(ptLayer);
  map.setView(config.initLatLng, config.initZoom);

  addRandomCoords(map.getBounds(),ptLayer,2000);
  ...
  $.each(routeLines, function(i, routeLine) {
    var marker = L.animatedMarker(routeLine.getLatLngs(), {
      ...
      onEnd: function() {
        ...
        var pt = L.GeometryUtil.closestLayer(map,ptLayer.getLayers(), e.latlng);
...
function playFrequency(frequency){
  ...
  vca.gain.value = 1;
  //window.setInterval(function (){vca.gain.value = 0;}, 5000);
}

function addRandomCoords(bounds,layer,numCoords){
  var markerOpts = {
      radius: 5,
      fillColor: "#ff7800",
      color: "#000",
  ...
  var colors = [ "#1f77b4", "#ff7f0e","#2ca02c","#d62728","#9467bd","#8c564b", "#e377c2"];
  var freqs = [ 440.0,392.0,349.23,329.63,293.66,261.63,246.94];
  for(i=0; i<numCoords; i++){
    markerOpts.fillColor = colors[i%7];
    markerOpts.frequency = freqs[i%7];
    L.circleMarker(randomCoord(bounds),markerOpts).addTo(layer);
  }
}

function randomCoord(bounds){
  return [random(bounds.getSouth(),bounds.getNorth()),
     random(bounds.getWest(),bounds.getEast())];
}
